...
function ViewAction({ dicomUid, status, date }: ViewActionProps) {
	...
	const archived = Number(date) < 20260205 // Orthanc that crashed and is now archived with images before this date

	const handleOpen = () => {
		if (status === 0 || (isGuest && status != 4)) {
			window.open(`https://radiology.mtrh.go.ke/images${archived ? "-archive" : ""}/viewer?StudyInstanceUIDs=${dicomUid}`)
		}
		else {
